...
public class HaveAddFlowCommand extends ModelCommand
{
   ...
   public boolean preCheck(WorkFlowsEditor editor) { 
      ...
      RemoveCommand oldRemove = editor.getRemoveCommands().get("WorkFlowsAddFlow-" + this.getId());
      if (oldRemove != null) {
         return false;
      }
      ModelCommand oldCommand = editor.getActiveCommands().get("WorkFlowsAddFlow-" + this.getId());
      if (oldCommand != null && java.util.Objects.compare(oldCommand.getTime(), this.getTime(), (a,b) -> a.compareTo(b)) >= 0) {
         return false;
      }
      editor.getActiveCommands().put("WorkFlowsAddFlow-" + this.getId(), this);
      return true;
   }

@Override
   public WorkFlowsAddFlow run(WorkFlowsEditor editor) { 
      if ( ! preCheck(editor)) {
         return editor.getWorkFlowsAddFlows().get(this.getId());
      }
      WorkFlowsAddFlow dataObject = editor.getOrCreateWorkFlowsAddFlow(this.getId());
